...
from data.twitter_interface.twitter_api import TimelineStatuses
...
def _add_to_users(user_list):
    """Used for recursively getting user models"""
    with USER_LOCK:
        for i in user_list:
            USERS.append(i)
...
def _user_mentions(user):
    from data.twitter_interface.twitter_api import db_limit_lock
...
def self_contained_network(untrimmed_collection, trimmed_collection, type=_from_static_list, user_list=_user_list,
    ...
    from data.db_interface import WriteToDatabase
